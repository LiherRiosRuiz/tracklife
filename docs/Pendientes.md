...
- [ ] **Endpoints sin UI que quedan (deliberado)** — `[2026-09-15]` `GET /api/workouts/{id}`, `/api/clubs/{id}`, `/api/challenges/{id}`, `/api/recipes/{id}`, `/api/activities/{id}` (páginas de detalle), `PUT /api/exercises/{id}` (editar ejercicio propio) y `/api/health` (sonda de infra). Son endpoints REST defendibles sin necesidad de UI hoy; `progreso` ya muestra el desglose por serie que justificaría el detalle de workout.
