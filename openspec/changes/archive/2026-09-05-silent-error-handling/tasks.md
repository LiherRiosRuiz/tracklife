...
- [x] 7.1 Manual, `planes/nuevo/page.tsx`: verified during PR3's apply batch via a real headless-Chromium/CDP session against an isolated dev server (offline network forced) — "Error al guardar el plan" rendered inline, form data preserved, no console-only failure. Re-confirmed present on `master` post-merge (code unchanged since).
- [x] 7.2 Manual, `favoritos/page.tsx`: verified live against the shared dev stack (`app.tracklife.test`, real Chromium via raw CDP) post-merge — registered a throwaway user, seeded one meal for a "Mas usados" candidate, forced the API offline, clicked the star: "Error al actualizar el favorito" rendered, optimistic star reverted correctly (count unchanged). Migration effect untouched (confirmed byte-identical in PR6's verify pass).
- [x] 7.3 Manual: expired session (401) sweep, live against the merged app — deleted the httpOnly session cookie via CDP `Network.deleteCookies` (can't be forged via `document.cookie`, so used protocol-level access) and loaded `/app/entrenamiento/planes`, `/app/entrenamiento/planes/<nonexistent-id>`, and `/app/nutricion/favoritos`. All three redirected cleanly to `/login`; no `.text-danger` text ever appeared in a 1.6s sampled window on any of them. D5 holds.
- [x] 7.4 `npm test` (59/59), `npm run lint` (0 errors, 7 pre-existing unrelated warnings), `npm run build` (clean) — all run directly on `master`'s tip post-merge (commit 76a7947). `rg` for `.catch(console.error)` / bare `catch { console.error }` across all six in-scope files: zero matches.
