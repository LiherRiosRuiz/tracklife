...
- [x] 6.5 HTTPS still inert: ran a real `make down && make up` full-stack cycle — grepped Traefik logs for acme certificate/obtain/renew/issue activity (none found); no `Location` redirect to HTTPS on any `*.test` host on port 80.
